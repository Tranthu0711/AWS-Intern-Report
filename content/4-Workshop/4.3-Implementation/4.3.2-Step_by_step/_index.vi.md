...
<br>&emsp; Bước 1: Tạo EC2 (server chạy web)
<br>&emsp; Bước 2: SSH + Cài đặt Apache + PHP
<br>&emsp; Bước 3: Upload code lên EC2
<br>&emsp; Bước 4: Copy vào web root
<br>&emsp; Bước 5: Kết nối cơ sở dư liệu (RDS)
<br>&emsp; Bước 6: S3 (upload ảnh)
...
1. SSH vào server
<br>Mở terminal (hoặc Git Bash)
<br> -  Bước 1: Di chuyển đến file .pem (Ví dụ (Windows Git Bash):  ```cd Downloads``` )
<br> -  Bước 2: Set quyền cho key
```chmod 400 aws-key.pem``` 
<br> -  Bước 3:  SSH vào server
```ssh -i aws-key.pem ec2-user@<public-ip>``` <br>
Ví dụ: ```ssh -i aws-key.pem ec2-user@<public-ip>```

2. Cập nhật server ```sudo dnf update -y```

3. Cài Apache + PHP ```sudo dnf install httpd php php-mysqlnd -y```
...
<br>Mở trình duyệt: http://public-ip <br>
**Kết quả đúng**
...
- Bước 6: Connectivity
<br>&emsp; + Public access → Chọn Yes
...
[**Phần 4: Import database**]: Cách đưa DB từ máy lên RDS
...
<br>&emsp; File views/pages/admin/modalBox.php
...
- B5.2: Upload ảnh trước bằng AJAX <br>
&emsp; Bỏ vào file: controllers/albumController.js
```
...
- B5.3: Gửi link S3 về backend <br>
&emsp; Bỏ vào file: albumController.js
...
- B5.4: Lưu vào cơ sở dữ liệu <br>
&emsp; Bỏ vào File: util/album.php
