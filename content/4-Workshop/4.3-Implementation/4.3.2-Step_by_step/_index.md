...
The deployment process includes the following steps: <br>
&emsp; Step 1: Create EC2 (Web Server) <br>
&emsp; Step 2: SSH & Install Apache + PHP <br>
&emsp; Step 3: Upload Code to EC2 <br>
&emsp; Step 4: Copy to Web Root <br>
&emsp; Step 5: Connect Database (RDS) <br>
&emsp; Step 6: S3 (Image Upload)
...
- Enable: 
<br>&emsp; Allow HTTP traffic (port 80) 
<br>&emsp; Allow HTTPS traffic (port 443) 
...
- Explanation: 
<br>&emsp; + Classic-Groove: the project folder on the local machine 
<br>&emsp; + /home/ec2-user/: destination directory on the EC2 server 
...
<br>&emsp; + Open a web browser and access: http://13.211.255.184
...
- Step 6: Connectivity
<br>&emsp; + Public access → Select Yes
...
[**Part 6: Restart**]
<br>```sudo systemctl restart httpd```
...
```
2. Create file upload - upload.php
```
cd /var/www/html
nano upload.php
```
...
**TEST:** Open browser: http://13.211.255.182/upload.php → It will show a blank page (Normal)

3. Create test upload form
...
- B5.1: Modify add album form File views/pages/admin/modalBox.php
...
- B5.2: Upload image via AJAX first → file: controllers/albumController.js
...
- B5.3: Send S3 URL to backend → file: albumController.js
...
- B5.4: Store in database → File: util/album.php
...
- B5.5: Display image from S3
<br>Add to: 
<br>&emsp; views/pages/user/home.php
<br>&emsp; views/pages/admin/productManager.php
